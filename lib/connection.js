...
class Connection {
    ...
    async saveDraft(data) {
        ...
        let draftsMailbox = await this.getSpecialUseMailbox('\\Drafts');
        if (!draftsMailbox) {
            // Try common Drafts folder names
            const candidates = ['Drafts', 'Draft', 'INBOX.Drafts'];
            for (let candidate of candidates) {
                try {
                    let status = await this.imapClient.status(candidate, { messages: true });
                    if (status) {
                        draftsMailbox = { path: candidate };
                        break;
                    }
                } catch (e) {
        ...
        if (!draftsMailbox) {
            let err = new Error('Drafts folder not found');
            err.code = 'DraftsFolderMissing';
            err.statusCode = 404;
            throw err;
        }

        let appendResult = await this.imapClient.append(draftsMailbox.path, raw, ['\\Draft', '\\Seen']);

        return {
            path: draftsMailbox.path,
            uid: appendResult && appendResult.uid,
            saved: true
        };
    }
